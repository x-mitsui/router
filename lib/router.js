...
Router.prototype.use = function () {
  ...
  for (let i = 0; i < middleware.length; i++) {
    ...
    if (m.router) {
      ...
      // Note Object.assign(target, ...source);//将各source可枚举属性分配到target，并返回target
      ...
      // 路由级别中间件 注册一个没有method但有中间件m的【路由表】实例
      // confuse路由级别中间件是为路由的每一张路由表服务么？
...
Router.prototype.routes = Router.prototype.middleware = function () {
  ...
  // 将router绑定到此中间件上，【Router.prototype.use中有用来判断是否为嵌套路由】
  dispatch.router = this;
  // todo待验证思考：是不是一旦执行，所有在此处构建的中间件链都会执行一遍
  return dispatch;
};
...
// 根据路径和请求方式，返回满足三种等级条件的路由表信息
Router.prototype.match = function (path, method) {
  const layers = this.stack;
  let layer;
  // 匹配对象分为三个级别：
  // 级别一数组：仅路径匹配；
  // 级别二数组：路径和请求方式匹配，但是包含拥有路由级别中间件的路由表；
  // 级别三标志：路径和请求方式匹配，不包含拥有路由级别中间件的路由表；
  const matched = {
    path: [],
    pathAndMethod: [],
    route: false,
  ...
  for (let len = layers.length, i = 0; i < len; i++) {
    ...
    if (layer.match(path)) {
      ...
      // 隐含：if两个条件成立事件绝对互斥，要么1，要么2；当然最后等级二数组可能包含满足1和满足2的
      if (layer.methods.length === 0 || ~layer.methods.indexOf(method)) {
        matched.pathAndMethod.push(layer);
        // 访问http://localhost:3001/foo/3 可以同时匹配get方式"/foo/:id"和"/foo/3"
        // 级别三标志：级别二数组保存的路由表中存在完全匹配path和请求方法method
