...
Router.prototype.match = function (path, method) {
  ...
  // 匹配对象分为三个级别：
  // 级别一：仅路径匹配；
  // 级别二：路径和动作匹配，但是包含拥有路由级别中间件的路由表；
  // 级别三：路径和动作匹配，不包含拥有路由级别中间件的路由表；
  const matched = {
    path: [],
    pathAndMethod: [],
    route: false,
  ...
  for (let len = layers.length, i = 0; i < len; i++) {
    ...
    if (layer.match(path)) {
      ...
      // 没有动作method，该【路由表】保存的为路由级别中间件
      // 要么保存的动作methods包含这个method - 路由请求方法也被匹配
      // 翻译：收集路径和方法都匹配的，当然methods长度为0，作者规定了全匹配
